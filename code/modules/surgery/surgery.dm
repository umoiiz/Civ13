...
proc/do_surgery(mob/living/human/M, mob/living/human/user, obj/item/tool)
	...
	if (zone in M.op_stage.in_progress) //Can't operate on someone repeatedly.
		to_chat(user, "<span class='warning'>当手术已经在进行时,你无法对这个区域进行手术.</span>")
		return TRUE
	for (var/datum/surgery_step/S in surgery_steps)
		...
		if (S.tool_quality(tool, user) > 50)
				...
				if (prob(S.tool_quality(tool, user)))
					...
					if (user.Adjacent(M) && (tool in user.contents))			//or
						...
						if (!(tool in user.contents))
							to_chat(user, "<span class='warning'>你停止了手术.</span>")
							if (prob(5))
								S.fail_step(user, M, zone, tool)
						else if (!user.Adjacent(M))
							to_chat(user, "<span class='warning'>你必须靠近你的病人才能进行手术.</span>")
							if (prob(15))
								S.fail_step(user, M, zone, tool)
						else // This failing silently was a pain.
							to_chat(user, "<span class='warning'>你的手滑了!</span>")
							if (prob(8))
								S.fail_step(user, M, zone, tool)
					M.op_stage.in_progress -= zone 									// Clear the in-progress flag.
					if (ishuman(M))
			...
			to_chat(user, "你技能不足,无法用\the [tool]执行这个手术步骤.")
			return TRUE
	return FALSE
