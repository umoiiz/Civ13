/*
The contents of this file were originally licensed under CC-BY-NC-SA 3.0 as part of Goonstation(https://ss13.co).
However, /tg/station and derivative codebases have been granted the right to use this code under the terms of the AGPL.
The original authors are: cogwerks, pistoleer, spyguy, angriestibm, marquesas, and stuntwaffle.
If you make a derivative work from this code, you must include this notification header alongside it.
*/

/mob/living/proc/wrestling_help()
	set name = "Recall Teachings"
	set desc = "Remember how to wrestle."
	set category = "Wrestling"

	to_chat(usr, "<b><i>你绷紧肌肉,突然灵光一闪...</i></b>")
	to_chat(usr, "<span class='notice'>缠抱</span>:抓取.被动地给予你一个立即强力抓取某人的机会.并非总是成功.")
	to_chat(usr, "<span class='notice'>背摔</span>:卸除你正在抓取之人的武器.将你的目标背摔到地上.对其造成严重伤害,并使你和你的目标都倒在地上.")
	to_chat(usr, "<span class='notice'>高级抓取</span>:抓取.被动地在抓取某人时造成耐力伤害.")

/datum/martial_art/wrestling
	name = "Wrestling"
	id = "wrestling"
	var/datum/action/slam/slam = new/datum/action/slam()
	var/datum/action/throw_wrassle/throw_wrassle = new/datum/action/throw_wrassle()
	var/datum/action/kick/kick = new/datum/action/kick()
	var/datum/action/strike/strike = new/datum/action/strike()
	var/datum/action/drop/drop = new/datum/action/drop()

/datum/martial_art/wrestling/proc/check_streak(mob/living/human/A, mob/living/human/D)
	switch(streak)
		if("drop")
			streak = ""
			drop(A,D)
			return TRUE
		if("strike")
			streak = ""
			strike(A,D)
			return TRUE
		if("kick")
			streak = ""
			kick(A,D)
			return TRUE
		if("throw")
			streak = ""
			throw_wrassle(A,D)
			return TRUE
		if("slam")
			streak = ""
			slam(A,D)
			return TRUE
	return FALSE

/datum/action/slam
	name = "Slam (Cinch) - Slam a grappled opponent into the floor."
	button_icon_state = "wrassle_slam"

/datum/action/slam/Trigger()
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>你在昏迷不醒时无法进行摔跤.</span>")
		return
	owner.visible_message("<span class='danger'>[owner]准备进行身体重压!</span>", "<b><i>你的下一次攻击将是身体重压.</i></b>")
	owner.mind.martial_art.streak = "slam"

/datum/action/throw_wrassle
	name = "Throw (Cinch) - Spin a cinched opponent around and throw them."
	button_icon_state = "wrassle_throw"

/datum/action/throw_wrassle/Trigger()
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>你在昏迷不醒时无法进行摔跤.</span>")
		return
	owner.visible_message("<span class='danger'>[owner]准备进行投掷!</span>", "<b><i>你的下一次攻击将是投掷.</i></b>")
	owner.mind.martial_art.streak = "throw"

/datum/action/kick
	name = "Kick - A powerful kick, sends people flying away from you. Also useful for escaping from bad situations."
	button_icon_state = "wrassle_kick"

/datum/action/kick/Trigger()
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>你在昏迷不醒时无法进行摔跤.</span>")
		return
	owner.visible_message("<span class='danger'>[owner]准备进行踢击!</span>", "<b><i>你的下一次攻击将是踢击.</i></b>")
	owner.mind.martial_art.streak = "kick"

/datum/action/strike
	name = "Strike - Hit a neaby opponent with a quick attack."
	button_icon_state = "wrassle_strike"

/datum/action/strike/Trigger()
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>你在昏迷不醒时无法进行摔跤.</span>")
		return
	owner.visible_message("<span class='danger'>[owner]准备进行打击!</span>", "<b><i>你的下一次攻击将是打击.</i></b>")
	owner.mind.martial_art.streak = "strike"

/datum/action/drop
	name = "Drop - Smash down onto an opponent."
	button_icon_state = "wrassle_drop"

/datum/action/drop/Trigger()
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>你在昏迷不醒时无法进行摔跤.</span>")
		return
	owner.visible_message("<span class='danger'>[owner]准备进行飞身压腿!</span>", "<b><i>你的下一次攻击将是飞身压腿.</i></b>")
	owner.mind.martial_art.streak = "drop"

/datum/martial_art/wrestling/teach(mob/living/owner, make_temporary=FALSE)
	if(..())
		to_chat(owner, "<span class='userdanger'>打起精神来!</span>")
		to_chat(owner, "<span class='danger'>将光标悬停在屏幕顶部的招式上以查看其效果.</span>")
		drop.Grant(owner)
		kick.Grant(owner)
		slam.Grant(owner)
		throw_wrassle.Grant(owner)
		strike.Grant(owner)

/datum/martial_art/wrestling/remove(mob/living/owner)
	drop.Remove(owner)
	kick.Remove(owner)
	slam.Remove(owner)
	throw_wrassle.Remove(owner)
	strike.Remove(owner)
	..()

/datum/martial_art/wrestling/harm_act(mob/living/human/A, mob/living/human/D)
	if(check_streak(A,D))
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Punched with wrestling by [A.name] ([A.ckey])</font>"
		return 1
	..()

/datum/martial_art/wrestling/proc/throw_wrassle(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	if(!A.pulling || A.pulling != D)
		to_chat(A, "<span class='warning'>你需要将[D]牢牢锁住!</span>")
		return
	D.forceMove(A.loc)
	D.dir = get_dir(D, A)

	D.Stun(80)
	D.visible_message("<span class='danger'>[A]开始带着[D]旋转!</span>", \
					"<span class='userdanger'>You're spun around by [A]!</span>", "<span class='hear'>你听到激烈的拖拽声!</span>", null, A)
	to_chat(A, "<span class='danger'>你开始带着[D]旋转!</span>")
	A.emote("scream")

	for (var/i = 0, i < 20, i++)
		var/delay = 5
		switch (i)
			if (17 to INFINITY)
				delay = 0.25
			if (14 to 16)
				delay = 0.5
			if (9 to 13)
				delay = 1
			if (5 to 8)
				delay = 2
			if (0 to 4)
				delay = 3

		if (A && D)

			if (get_dist(A, D) > 1)
				to_chat(A, "<span class='warning'>[D]距离太远了!</span>")
				return

			if (!isturf(A.loc) || !isturf(D.loc))
				to_chat(A, "<span class='warning'>你无法从这里投掷[D]!</span>")
				return

			A.dir = turn(A.dir, 90)
			var/turf/T = get_step(A, A.dir)
			var/turf/S = D.loc
			if ((S && isturf(S) && S.Exit(D)) && (T && isturf(T) && T.Enter(A)))
				D.forceMove(T)
				D.dir = get_dir(D, A)
		else
			return

		sleep(delay)

	if (A && D)
		// These are necessary because of the sleep call.

		if (get_dist(A, D) > 1)
			to_chat(A, "<span class='warning'>[D]距离太远了!</span>")
			return

		if (!isturf(A.loc) || !isturf(D.loc))
			to_chat(A, "<span class='warning'>你无法从这里投掷[D]!</span>")
			return

		D.forceMove(A.loc) // Maybe this will help with the wallthrowing bug.

		D.visible_message("<span class='danger'>[A]投掷了[D]!</span>", \
						"<span class='userdanger'>You're thrown by [A]!</span>", "<span class='hear'>你听到激烈的拖拽声和一声巨响!</span>", null, A)
		to_chat(A, "<span class='danger'>你投掷了[D]!</span>")
		playsound(A.loc, "swing_hit", 50, TRUE)
		var/turf/T = get_edge_target_turf(A, A.dir)
		if (T && isturf(T))
			if (!D.stat)
				D.emote("scream")
			D.throw_at(T, 10, 4, A, TRUE, TRUE)
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Thrown with wrestling by [A.name] ([A.ckey])</font>"
	return

/datum/martial_art/wrestling/proc/FlipAnimation(mob/living/D)
	set waitfor = FALSE
	if (D)
		animate(D, transform = matrix(180, MATRIX_ROTATE), time = 1, loop = 0)
	sleep(15)
	if (D)
		animate(D, transform = null, time = 1, loop = 0)

/datum/martial_art/wrestling/proc/slam(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	if(!A.pulling || A.pulling != D)
		to_chat(A, "<span class='warning'>你需要将[D]牢牢锁住!</span>")
		return
	D.forceMove(A.loc)
	A.dir = get_dir(A, D)
	D.dir = get_dir(D, A)

	D.visible_message("<span class='danger'>[A]将[D]举了起来!</span>", \
					"<span class='userdanger'>You're lifted up by [A]!</span>", "<span class='hear'>你听到激烈的拖拽声!</span>", null, A)
	to_chat(A, "<span class='danger'>你将[D]举了起来!</span>")

	FlipAnimation()

	for (var/i = 0, i < 3, i++)
		if (A && D)
			A.pixel_y += 3
			D.pixel_y += 3
			A.dir = turn(A.dir, 90)
			D.dir = turn(D.dir, 90)

			switch (A.dir)
				if (NORTH)
					D.pixel_x = A.pixel_x
				if (SOUTH)
					D.pixel_x = A.pixel_x
				if (EAST)
					D.pixel_x = A.pixel_x - 8
				if (WEST)
					D.pixel_x = A.pixel_x + 8

			if (get_dist(A, D) > 1)
				to_chat(A, "<span class='warning'>[D]距离太远了!</span>")
				A.pixel_x = 0
				A.pixel_y = 0
				D.pixel_x = 0
				D.pixel_y = 0
				return

			if (!isturf(A.loc) || !isturf(D.loc))
				to_chat(A, "<span class='warning'>你无法从这里重压[D]!</span>")
				A.pixel_x = 0
				A.pixel_y = 0
				D.pixel_x = 0
				D.pixel_y = 0
				return
		else
			if (A)
				A.pixel_x = 0
				A.pixel_y = 0
			if (D)
				D.pixel_x = 0
				D.pixel_y = 0
			return

		sleep(1)

	if (A && D)
		A.pixel_x = 0
		A.pixel_y = 0
		D.pixel_x = 0
		D.pixel_y = 0

		if (get_dist(A, D) > 1)
			to_chat(A, "<span class='warning'>[D]距离太远了!</span>")
			return

		if (!isturf(A.loc) || !isturf(D.loc))
			to_chat(A, "<span class='warning'>你无法从这里重压[D]!</span>")
			return

		D.forceMove(A.loc)

		var/fluff = "body-slam"
		switch(pick(2,3))
			if (2)
				fluff = "turbo [fluff]"
			if (3)
				fluff = "atomic [fluff]"

		D.visible_message("<span class='danger'>[A] [fluff] [D]!</span>", \
						"<span class='userdanger'>You're [fluff]ed by [A]!</span>", "<span class='hear'>你听到一阵令人作呕的皮肉相击声!</span>")
		to_chat(A, "<span class='danger'>你[fluff][D]!</span>")
		playsound(A.loc, "swing_hit", 50, TRUE)
		if (!D.stat)
			D.emote("scream")
			D.Paralyse(40)

			switch(rand(1,3))
				if (2)
					D.adjustBruteLoss(rand(20,30))
				if (3)
					D.ex_act(3)
				else
					D.adjustBruteLoss(rand(10,20))
		else
			D.ex_act(3)

	else
		if (A)
			A.pixel_x = 0
			A.pixel_y = 0
		if (D)
			D.pixel_x = 0
			D.pixel_y = 0

	D.attack_log += "\[[time_stamp()]\] <font color='orange'>Body-slammed by [A.name] ([A.ckey])</font>"
	return

/datum/martial_art/wrestling/proc/CheckStrikeTurf(mob/living/A, turf/T)
	if (A && (T && isturf(T) && get_dist(A, T) <= 1))
		A.forceMove(T)

/datum/martial_art/wrestling/proc/strike(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	var/turf/T = get_turf(A)
	if (T && isturf(T) && D && isturf(D.loc))
		for (var/i = 0, i < 4, i++)
			A.dir = turn(A.dir, 90)

		A.forceMove(D.loc)
		addtimer(CALLBACK(src, src.CheckStrikeTurf(), A, T), 4)

		D.visible_message("<span class='danger'>[A]用头撞向[D]!</span>", \
						"<span class='userdanger'>You're headbutted by [A]!</span>", "<span class='hear'>你听到一阵令人作呕的皮肉相击声!</span>")
		to_chat(A, "<span class='danger'>你用头撞向[D]!</span>")
		D.adjustBruteLoss(rand(10,20))
		playsound(A.loc, "swing_hit", 50, TRUE)
		D.Paralyse(20)
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Headbutted with wrestling by [A.name] ([A.ckey])</font>"

/datum/martial_art/wrestling/proc/kick(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	A.emote("scream")
	A.emote("flip")
	A.dir = turn(A.dir, 90)

	D.visible_message("<span class='danger'>[A]回旋踢向[D]!</span>", \
					"<span class='userdanger'>You're roundhouse-kicked by [A]!</span>", "<span class='hear'>你听到一阵令人作呕的皮肉相击声!</span>")
	to_chat(A, "<span class='danger'>你回旋踢向[D]!</span>")
	playsound(A.loc, "swing_hit", 50, TRUE)
	D.adjustBruteLoss(rand(10,20))

	var/turf/T = get_edge_target_turf(A, get_dir(A, get_step_away(D, A)))
	if (T && isturf(T))
		D.Paralyse(20)
		D.throw_at(T, 3, 2)
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Roundhouse-kicked by [A.name] ([A.ckey])</font>"

/datum/martial_art/wrestling/proc/drop(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	var/obj/surface = null
	var/turf/ST = null
	var/falling = 0

	for (var/obj/O in oview(1, A))
		if (O.density == 1)
			if (O == A)
				continue
			if (O == D)
				continue
			if (O.opacity)
				continue
			else
				surface = O
				ST = get_turf(O)
				break

	if (surface && (ST && isturf(ST)))
		A.forceMove(ST)
		A.visible_message("<span class='danger'>[A]爬上了[surface]!</span>", \
						"<span class='danger'>You climb onto [surface]!</span>")
		A.pixel_y = 0 + 10
		falling = 1
		sleep(10)

	if (A && D)
		// These are necessary because of the sleep call.

		if ((falling == 0 && get_dist(A, D) > 1) || (falling == 1 && get_dist(A, D) > 2)) // We climbed onto stuff.
			A.pixel_y = 0
			if (falling == 1)
				A.visible_message("<span class='danger'>...然后头朝下栽进了地面,哎哟!</span>", \
								"<span class='userdanger'>...and dive head-first into the ground, ouch!</span>")
				A.adjustBruteLoss(rand(10,20))
				A.Paralyse(60)
			to_chat(A, "<span class='warning'>[D]距离太远了!</span>")
			return

		if (!isturf(A.loc) || !isturf(D.loc))
			A.pixel_y = 0
			to_chat(A, "<span class='warning'>你无法从这里扑向[D]!</span>")
			return

		if(A)
			animate(A, transform = matrix(90, MATRIX_ROTATE), time = 1, loop = 0)
		sleep(10)
		if(A)
			animate(A, transform = null, time = 1, loop = 0)

		A.forceMove(D.loc)

		D.visible_message("<span class='danger'>[A]飞身压腿砸向[D]!</span>", \
						"<span class='userdanger'>You're leg-dropped by [A]!</span>", "<span class='hear'>你听到一阵令人作呕的皮肉相击声!</span>", null, A)
		to_chat(A, "<span class='danger'>你飞身压腿砸向[D]!</span>")
		playsound(A.loc, "swing_hit", 50, TRUE)
		A.emote("scream")
		D.adjustBruteLoss(rand(20,30))

		D.Paralyse(40)

		A.pixel_y = 0

	else
		if (A)
			A.pixel_y = 0
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Legdropped by [A.name] ([A.ckey])</font>"
	return

/datum/martial_art/wrestling/disarm_act(mob/living/human/A, mob/living/human/D)
	if(check_streak(A,D))
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>disarmed with wrestling by [A.name] ([A.ckey])</font>"
		return 1
	..()

/datum/martial_art/wrestling/grab_act(mob/living/human/A, mob/living/human/D)
	if(check_streak(A,D))
		return 1
	if(A.pulling == D)
		return 1
	A.start_pulling(D)
	D.visible_message("<span class='danger'>[A]将[D]牢牢锁住!</span>", \
					"<span class='userdanger'>You're put into a cinch by [A]!</span>", "<span class='hear'>你听到激烈的拖拽声!</span>")
	to_chat(A, "<span class='danger'>你将[D]牢牢锁住!</span>")
	D.Stun(rand(60,100))
	D.attack_log += "\[[time_stamp()]\] <font color='orange'>Cinched by [A.name] ([A.ckey])</font>"
	return 1
