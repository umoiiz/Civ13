...
/datum/admins/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	...
	if (!usr.client.holder || !(usr.client.holder.rights & R_PERMISSIONS))
		to_chat(usr, "<span class = 'red'>你没有权限执行此操作!</span>")
		return
	...
	if (new_rank == "Removed")
		...
		if (!foundnew)
			...
			to_chat(usr, "<span class = 'good'>新管理员已成功添加.</span>")
			return
		else
			...
			to_chat(usr, "<span class = 'good'>管理员等级已成功更改.</span>")
			return
// see admin/topic.dm
/datum/admins/proc/log_admin_permission_modification(var/adm_ckey, var/new_permission, var/nominal)
	...
	if (admin_rights & new_permission) //This admin already has this permission, so we are removing it.
		...
		to_chat(usr, "<span class = 'notice'>权限已移除.</span>")
		...
		to_chat(usr, "<span class = 'notice'>权限已添加.</span>")
	return