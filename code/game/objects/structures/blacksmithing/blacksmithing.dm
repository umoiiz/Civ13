/obj/structure/anvil
	name = "铁砧"
	desc = "一个沉重的铁砧.铁匠的主要工作工具.上面有0块热铁锭."
	...
	if (map.ID == MAP_FOUR_KINGDOMS && (H.gorillaman || H.ant || H.wolfman || H.lizard || H.crab))
		...
		if (istype(P, /obj/item/stack/material))
			...
			if (!(MTR.default_type in accepted_materials))
				to_chat(user, "<span class='warning'>你不能在这个铁砧上使用这种材料.</span>")
				return
		if (istype(P, /obj/item/stack/ore/iron_pig))
			if (!("iron" in accepted_materials))
				to_chat(user, "<span class='warning'>你不能在这个铁砧上使用这种材料.</span>")
				return
			to_chat(user, "你开始锻造生铁...")
			icon_state = "[base_icon]_anvil_use"
			in_use = TRUE
			update_icon()
			...
			if (do_after(user,15*initial_amount,src) && P.amount == initial_amount)
				to_chat(user, "<span class='notice'>你将生铁锻造成了钢.</span>")
			...
			var/initial_amount = P.amount //no more speedhack
			if (do_after(user,15*initial_amount,src) && P.amount == initial_amount)
				to_chat(user, "<span class='notice'>你将海绵铁锻造成了熟铁.</span>")
				if (P && P.amount == initial_amount)
					var/amt = P.amount
					qdel(P)
			...
			if (H.orc)
				...
				if (anvil_recipes[parsed_choice2[1]])
					if (P.amount >= anvil_recipes[parsed_choice2[1]][6])
						to_chat(user, "你开始制作\the [parsed_choice2[1]]...")
						in_use = TRUE
						update_icon()
						if (do_after(user,10*anvil_recipes[parsed_choice2[1]][6],src,can_move=FALSE))
							if (P.amount >= anvil_recipes[parsed_choice2[1]][6])
								...
								if (P.amount <= 0)
									qdel(P)
								to_chat(user, "你完成了制作\the [parsed_choice2[1]].")
...
/////////BRONZE//////////
			...
			if (!ML.fired)
				to_chat(user, "<span class='warning'>这个模具没有烧制过!</span>")
				return
			else if (ML.capacity <= 0 || ML.current_material == null)
				to_chat(user, "<span class='warning'>这个模具是空的!</span>")
				return
				...
				switch(ML.craftable_classes)
						...
						if (anvil_recipes[parsed_choice2[1]])
							...
							if (ML.capacity >= mat)
								...
								if (do_after(user,10*mat,src,can_move=FALSE))
									if (ML.capacity >= mat)
										ML.capacity -= mat
										to_chat(user, "你完成了\the [parsed_choice2[1]]的制作.")
					...
					if ("swords")
						...
						if (anvil_recipes[parsed_choice2[1]])
							...
							if (ML.capacity >= mat)
								...
								return
					if ("spearheads")
						to_chat(user, "你开始制作[ML.current_material]矛头...")
						in_use = TRUE
						update_icon()
						if (do_after(user,10*ML.capacity,src,can_move=FALSE))
							if (ML && ML.capacity)
								...
								if (tamt >= 1)
									...
									ML.current_material = null
									to_chat(user, "你完成了[ML.current_material]矛头的制作.")
					...
					if ("pickaxes")
						to_chat(user, "你开始制作[ML.current_material]镐头...")
						in_use = TRUE
						update_icon()
						if (do_after(user,10*ML.capacity,src,can_move=FALSE))
							if (ML && ML.capacity)
								...
								if (tamt >= 1)
									...
									ML.current_material = null
									to_chat(user, "你完成了[ML.current_material]镐头的制作.")
						...
						return
					if ("shovels")
						to_chat(user, "你开始制作[ML.current_material]铲头...")
						in_use = TRUE
						update_icon()
						if (do_after(user,10*ML.capacity,src,can_move=FALSE))
							if (ML && ML.capacity)
								...
								if (tamt >= 1)
									...
									ML.current_material = null
									to_chat(user, "你完成了[ML.current_material]铲头的制作.")
					...
					if ("arrowheads")
						if (ML.current_material in list("copper", "bronze", "iron", "steel"))
							to_chat(user, "你开始制作[ML.current_material]箭头...")
							in_use = TRUE
							update_icon()
							if (do_after(user,10*ML.capacity,src,can_move=FALSE))
								if (ML && ML.capacity)
									...
									ML.current_material = null
									ML.update_icon()
									to_chat(user, "你完成了[ML.current_material]箭头的制作.")
