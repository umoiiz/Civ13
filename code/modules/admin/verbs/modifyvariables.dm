...
/client/proc/mod_list(var/list/L, atom/O, original_name, objectvar)
	...
	if (!istype(L,/list)) to_chat(src, "不是列表.")
...
/client/proc/modify_variables(var/atom/O, var/param_var_name = null, var/autodetect_class = FALSE)
	...
	for (var/p in forbidden_varedit_object_types)
		if ( istype(O,p) )
			to_chat(usr, "<span class='danger'>禁止编辑此对象的变量.</span>")
			return
	...
	if (param_var_name)
		if (!(param_var_name in O.vars))
			to_chat(src, "此原子([O])中不存在名为([param_var_name])的变量")
			return
	...
	if (variable in list("lastKnownCID", "lastKnownCkey", "lastKnownIP"))
		to_chat(usr, "<span class = 'danger'>你无权编辑此变量.</span>")
		return

	if ((variable in list("ckey", "key")) && class != "text")
		to_chat(usr, "<span class = 'danger'>此变量的类型无效.</span>")
		return
		...
		if ("text")
			...
			if (usr && (variable in list("ckey", "key")) && (var_new in list(usr.ckey, usr.key)))
				if (!O.vars["lastKnownCkey"] || !(O.vars["lastKnownCkey"] == usr.ckey))
					to_chat(usr, "<span class = 'danger'>使用玩家面板将自己生成一个生物.</span>")
					return
