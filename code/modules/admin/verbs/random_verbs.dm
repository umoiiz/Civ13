...
/client/proc/cmd_mentor_check_new_players()	//Allows mentors / admins to determine who the newer players are.
	...
	if (missing_ages)
		to_chat(src, "某些账号在其客户端中没有设置正确的年龄. 此功能需要数据库存在.")

	if (msg != "")
		src << browse(msg, "window=Player_age_check")
	else
		to_chat(src, "未找到该年龄范围的匹配项.")
...
/client/proc/cmd_admin_godmode(mob/M as mob in mob_list)
	...
	to_chat(usr, "<span class = 'notice'>已切换[(M.status_flags & GODMODE) ? "ON" : "OFF"]</span>")
...
proc/cmd_admin_mute(mob/M as mob, mute_type, automute = FALSE)
	if (automute)
		...
		if (!usr || !usr.client)
			return
		if (!usr.client.holder)
			to_chat(usr, "<font color='red'>错误: cmd_admin_mute: 你无权执行此操作.</font>")
			return
		if (!M.client)
			to_chat(usr, "<font color='red'>错误: cmd_admin_mute: 此生物没有关联的客户端.</font>")
		if (M.client.holder)
			to_chat(usr, "<font color='red'>错误: cmd_admin_mute: 你不能禁言管理员/版主.</font>")
	if (!M.client)		return
	...
	if (automute)
		...
		to_chat(M, "<span class='alert'>你已被SPAM AUTOMUTE系统从[mute_string][muteunmute]. 请联系管理员.</span>")
	...
	to_chat(M, "<span class = 'alert'>你已被从[mute_string][muteunmute].</span>")
...
/client/proc/respawn_character()
	...
	if (!G_found)//If a ghost was not found.
		to_chat(usr, "<font color='red'>游戏中没有该活跃键,或者此人当前不是幽灵.</font>")
		return
...
/client/proc/toggle_right_click()
	...
	if(!show_popup_menus)
		...
		to_chat(src, "<span class='interface'>右键点击已启用.</span>")
	else
		show_popup_menus = FALSE
		to_chat(src, "<span class='interface'>右键点击已禁用.</span>")
