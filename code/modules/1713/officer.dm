...
/mob/living/human/proc/find_nco()
	...
	if (faction_text == map.faction1)
		if (!map.faction1_squad_leaders[squad])
			to_chat(src, "<big>没有小队队长!</big>")
			return
		else if (map.faction1_squad_leaders[squad] == src)
			to_chat(src, "<big>你就是小队队长!</big>")
			return
	...
	if (TSL)
		...
		to_chat(src, "<big><font color='yellow'>你的小队队长在你[tdir][tdist]米处.</font></big>")
...
/mob/living/human/proc/Check_Coordinates_Chump()
	...
	if (!check_coords_check())
		to_chat(usr, "<span class = 'warning'>你现在还不能使用这个.</span>")
		return
	if (checking_coords[1] && checking_coords[2])
		...
		to_chat(usr, "<span class = 'notice'>你完成了坐标追踪,位于<b>[x],[y]</b>.你移动了<b>[dist]</b>的偏移量.[kanonier_msg]</span>")
		checking_coords[3] = null
		checking_coords[4] = null // continue to track from the same starting location
	else
		checking_coords[1] = x
		checking_coords[2] = y
		to_chat(usr, "<span class = 'notice'>你开始检查坐标,位于<b>[x],[y]</b>.</span>")
...
/mob/living/human/proc/Reset_Coordinates_Chump()
	...
	if (checking_coords[1] && checking_coords[2])
		...
		to_chat(usr, "<span class = 'warning'>你不再从<b>[x],[y]</b>进行追踪.</span>")
		checking_coords[3] = null
...
/mob/living/human/RangedAttack(var/turf/t)
	if (checking_coords[1] && istype(t))
		if (can_check_distant_coordinates && get_turf(src) != t)
			var/offset_x = t.x - x
			var/offset_y = t.y - y
			to_chat(src, "<span class = 'notice'>此地块的偏移量为<b>[offset_x],[offset_y]</b>,坐标为<b>[t.x],[t.y]</b>.[kanonier_msg]</span>")
...
/mob/living/human/proc/order_airstrike()
	...
	if (map.artillery_count > 0 && world.time >= map.artillery_last+map.artillery_timer)
		...
		if (map.ID == MAP_GROZNY)
			...
			if (input1 == "Cancel")
				...
				if (!valid_coords_check)
					to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>炮兵后方阵地,[currfreq]千赫:</font></b><font size=2]><span class = 'small_message'>([default_language.name])</span>\"否定,[name],我重复,否定.那些坐标并非由侦察兵或军官报告.完毕.\"</font>")
					return
				for (var/mob/living/human/friendlies in range(7, locate(inputx,inputy,src.z)))
					if (friendlies.faction_text == faction_text && friendlies.stat != DEAD)
						to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>炮兵后方阵地,[currfreq]千赫:</font></b><font size=2]><span class = 'small_message'>([default_language.name])</span>\"否定,[name],我重复,否定.该区域有友军.请选择其他区域.完毕.\"</font>")
						return
				to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>炮兵后方阵地,[currfreq]千赫:</font></b><font size=2><span class = 'small_message'>([default_language.name])</span>\"收到,[name],[input1]炮击正前往[inputx],[inputy].请保持距离.完毕.\"</font>")
					...
					for (var/coords in global.valid_coordinates)
						...
						if (abs(coordx - inputx) <= 15)
							if (abs(coordy - inputy) <= 15)
								valid_coords_check = TRUE
				if (!valid_coords_check)
					to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>空军指挥部,[currfreq]千赫:</font></b><font size=2]><span class = 'small_message'>([default_language.name])</span>\"否定,[name],我重复,否定.那些坐标并非由侦察兵或军官报告.完毕.\"</font>")
					return
				for (var/mob/living/human/friendlies in range(7, locate(inputx,inputy,src.z)))
					if (friendlies.faction_text == faction_text && friendlies.stat != DEAD)
						to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>空军指挥部,[currfreq]千赫:</font></b><font size=2]><span class = 'small_message'>([default_language.name])</span>\"否定,[name],我重复,否定.该区域有友军.请选择其他区域.完毕.\"</font>")
						return
				to_chat(src, "\icon[getFlatIcon(radio)]<font size=2 color=#FFAE19><b>空军指挥部,[currfreq]千赫:</font></b><font size=2><span class = 'small_message'>([default_language.name])</span>\"收到,[name],[input1]轰炸任务正前往[inputx],[inputy].请保持距离.完毕.\"</font>")
				playsound(loc, "radio_chatter", 250, FALSE)
				map.artillery_count--
				map.artillery_last = world.time
				spawn(rand(15,25)*10)
		...
		if (map.ID == MAP_GROZNY)
			to_chat(src, "<span class='warning'>没有更多可用的炮击了.</span>")
			return
		else
			to_chat(src, "<span class='warning'>没有更多可用的空袭了.</span>")
			return	
	else if (world.time < map.artillery_last+map.artillery_timer)
		if (map.ID == MAP_GROZNY)
			to_chat(src, "<span class='warning'>你现在还不能下令炮击!等待[round(((map.artillery_last+map.artillery_timer)-world.time)/600)]分钟.</span>")
			return
		else
			to_chat(src, "<span class='warning'>你现在还不能下令空袭!等待[round(((map.artillery_last+map.artillery_timer)-world.time)/600)]分钟.</span>")
			return
...
/mob/living/human/proc/find_hvt()
	...
	for (var/mob/living/human/H in player_list)
		if (H.original_job_title == ("Soviet Supreme Chairman" || "US HVT"))
			...
			count++
			to_chat(src, "<big><font color='yellow'><b>高价值目标</b>(<i>[H.name]</i>)在你[tdir][tdist]米处.</font></big>")
	if (count <= 0)
		to_chat(src, "<big><font color='yellow'>未发现<b>高价值目标</b>!</font></big>")
