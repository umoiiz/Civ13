...
/obj/map_metadata/nomads/wasteland/proc/nuke_proc(var/timer=72000)
	if (processes.ticker.playtime_elapsed > timer && hasnukes)
		...
		to_chat(world, "<font size=4 color='red'><center>注意<br>核导弹来袭! 寻找掩体!</center></font>")
		var/warning_sound = sound("sound/misc/siren.ogg", repeat = FALSE, wait = TRUE, channel = 777)
		for (var/mob/M in player_list)
			M.client << warning_sound
		spawn(330)
			to_chat(world, "<font size=4 color='red'>核爆炸已经发生! <br><i>(处理时游戏可能会卡顿/延迟一段时间, 请稍候)</i></font>")
...
/obj/map_metadata/nomads/wasteland/proc/supplydrop_proc()
	if ((global_radiation >= 280 && hasnukes)||is_zombie == TRUE)
		...
		switch(droptype)
			if("supplies")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>补给已空投至该区域!</center></font>")
				new/obj/structure/closet/crate/airdrops/supplies(locationt)

			if("food")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>食物和水已空投至该区域!</center></font>")
			...
			if("weapons")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>武器和弹药已空投至该区域!</center></font>")
				new/obj/structure/closet/crate/airdrops/weapons(locationt)

			if("military")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>军事装备已空投至该区域!</center></font>")
				new/obj/structure/closet/crate/airdrops/military(locationt)

			if("medicine")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>药品已空投至该区域!</center></font>")
				new/obj/structure/closet/crate/airdrops/medicine(locationt)

			if("rad")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>防辐射装备已空投至该区域!</center></font>")
				new/obj/structure/closet/crate/airdrops/rads(locationt)

			if("cold")
				to_chat(world, "<font size=3 color='red'><center>紧急广播系统<br>防寒装备已空投至该区域!</center></font>")
