...
/process/time_of_day_change/fire(var/special = FALSE)
	...
	if (admincaller)
		to_chat(admincaller, "<span class = 'notice'>更新了[time_of_day]的灯光.</span>")
	...
	if (announce)
		for (var/M in player_list)
			to_chat(M, "<font size=3><span class = 'notice'>现在是<b>[lowertext(capitalize(time_of_day))]</b>.</span></font>")
